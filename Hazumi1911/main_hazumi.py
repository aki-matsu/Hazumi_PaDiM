...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='./png')
    parser.add_argument('--save_path', type=str, default='./hazumi_result')
...
def main():
    ...
    for class_name in hazumi.CLASS_NAMES:
        # gitに用意しているプログラムの関数
        ## マスクもこの関数内で作成
        train_dataset = hazumi.HazumiDataset(args.data_path, class_name=class_name, is_train=True)
        train_dataloader = DataLoader(train_dataset, batch_size=32, pin_memory=True)
        test_dataset = hazumi.HazumiDataset(args.data_path, class_name=class_name, is_train=False)
